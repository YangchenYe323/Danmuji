...
use std::{sync::{Arc, atomic::{AtomicBool, Ordering}}, collections::HashMap, time::{Instant, UNIX_EPOCH, SystemTime}, thread::sleep_ms};
...
use crate::{config::{UserConfig, BulletScreenConfig, RoomConfig, Room}, USER_AGENT, DanmujiResult};
...
impl DanmujiSender {
	pub fn start(upstream: Producer) -> Self {
		...
		tokio::spawn(start_worker(upstream, shutdown.clone(), user.clone(), danmu.clone(), room.clone()));
...
async fn start_worker(
	...
	loop {
		...
		
		{
			...
			let res = cli.post("https://api.live.bilibili.com/msg/send")
...
fn build_form<'a>(msg: String, room: &'a RoomConfig, user: &'a UserConfig, danmu: &'a BulletScreenConfig) -> HashMap<&'static str, String> {
	...
	form.insert("msg", msg);
	let mut rnd = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis().to_string();
	rnd.truncate(10);
	form.insert("rnd", rnd);
