...
export function Message({
	...
	return (
		<div className="flex flex-wrap justify-items-start items-center animate-danmaku-movein font-mono h-8 min-h-fit">
			...
			{
				message.body.is_manager? <span className="border border-black bg-transparent rounded-sm text-xs text-emerald-500 font-serif mx-px">房</span> : null
			}
			{
			...
			<span className="text-white shadowed-text mx-px">{message.body.content}</span>
		</div>
	);
}